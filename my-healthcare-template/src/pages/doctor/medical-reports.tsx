...
export default function MedicalReports() {
  ...
  const fetchReports = async (patientId: number) => {
    const { data, error } = await supabase
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10">
      <h1 className="text-2xl font-bold mb-4">Lékařské zprávy</h1>
      ...
      {selectedPatient && (
        <section>
          <h2 className="text-xl font-semibold mb-2">Zprávy pro {selectedPatient.name}</h2>
          <ul>
            {reports.map((report) => (
